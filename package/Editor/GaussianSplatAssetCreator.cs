...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        [SerializeField] GaussianSplatAsset.MaterialFormat m_FormatMat = GaussianSplatAsset.MaterialFormat.Float32x2;
        ...
        void OnGUI()
        {
            ...
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            m_FormatMat = (GaussianSplatAsset.MaterialFormat)EditorGUILayout.EnumPopup("Material", m_FormatMat);
            GUILayout.Label(sizeCol > 0 ? EditorUtility.FormatBytes(sizeCol) : string.Empty, GUILayout.Width(kSizeColWidth));
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
        ...
        void ApplyQualityLevel()
        {
            switch (m_Quality)
            {
                ...
                case DataQuality.VeryLow: // 18.62x smaller, 32.27 PSNR
                    ...
                    m_FormatMat = GaussianSplatAsset.MaterialFormat.Norm8x2;
                ...
                case DataQuality.High: // 2.94x smaller, 57.77 PSNR
                    ...
                    m_FormatMat = GaussianSplatAsset.MaterialFormat.Float16x2;
                ...
                case DataQuality.VeryHigh: // 1.05x smaller
                    ...
                    m_FormatScale = GaussianSplatAsset.VectorFormat.Float32;
                    m_FormatColor = GaussianSplatAsset.ColorFormat.Float32x4;
                    m_FormatMat = GaussianSplatAsset.MaterialFormat.Float32x2;
                    m_FormatSH = GaussianSplatAsset.SHFormat.Float32;
                    break;
                default:
        ...
        unsafe void CreateAsset()
        {
            ...
            asset.Initialize(inputSplats.Length, m_FormatPos, m_FormatScale, m_FormatColor, m_FormatMat, m_FormatSH, boundsMin, boundsMax, cameras);
            asset.name = baseName;
            ...
            string pathNor = $"{m_OutputFolder}/{baseName}_nor.bytes";
            string pathOther = $"{m_OutputFolder}/{baseName}_oth.bytes";
            string pathCol = $"{m_OutputFolder}/{baseName}_col.bytes";
            string pathAlb = $"{m_OutputFolder}/{baseName}_alb.bytes";
            string pathMat = $"{m_OutputFolder}/{baseName}_mat.bytes";
            ...
            CreateNormalData(inputSplats, pathNor, ref dataHash);
            CreateOtherData(inputSplats, pathOther, ref dataHash, splatSHIndices);
            CreateColorData(inputSplats, pathCol, ref dataHash);
            CreateAlbedoData(inputSplats, pathAlb, ref dataHash);
            CreateMaterialData(inputSplats, pathMat, ref dataHash);
            ...
            asset.SetAssetFiles(
                ...
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathNor),
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathOther),
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathCol),
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathAlb),
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathMat),
        ...
        [BurstCompile]
        struct ConvertMaterialJob : IJobParallelFor
        {
            public int width, height;
            [ReadOnly] public NativeArray<float2> inputData;
            [NativeDisableParallelForRestriction] public NativeArray<byte> outputData;
            public GaussianSplatAsset.MaterialFormat format;
            public int formatBytesPerPixel;
            ...
            public unsafe void Execute(int y)
            {
                ...
                for (int x = 0; x < width; ++x)
                {
                    float2 pix = inputData[srcIdx];

                    switch (format)
                    {
                        case GaussianSplatAsset.MaterialFormat.Float32x2:
                            {
                                *(float2*)dstPtr = pix;
                            }
                            break;
                        case GaussianSplatAsset.MaterialFormat.Float16x2:
                            {
                                half2 enc = new half2(pix);
                                *(half2*)dstPtr = enc;
                            }
                            break;
                        case GaussianSplatAsset.MaterialFormat.Norm8x2:
                            {
                                pix = math.saturate(pix);
                                uint enc = (uint)(pix.x * 255.5f) | ((uint)(pix.y * 255.5f) << 8);
                                *(uint*)dstPtr = enc;
                            }
                            break;
                    }
        ...
        [BurstCompile]
        struct ConvertNormalJob : IJobParallelFor
        {
            public int width, height;
        ...
        [BurstCompile]
        struct CreateAlbedoDataJob : IJobParallelFor
        {
            [ReadOnly] public NativeArray<InputSplatData> m_Input;
            [NativeDisableParallelForRestriction] public NativeArray<float4> m_Output;
            ...
            public void Execute(int index)
            {
                var splat = m_Input[index];
                int i = SplatIndexToTextureIndex((uint)index);
                m_Output[i] = new float4(splat.albedo.x, splat.albedo.y, splat.albedo.z, splat.opacity);
            }
        }

        void CreateAlbedoData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            var (width, height) = GaussianSplatAsset.CalcTextureSize(inputSplats.Length);
            NativeArray<float4> data = new(width * height, Allocator.TempJob);

            CreateAlbedoDataJob job = new CreateAlbedoDataJob();
            job.m_Input = inputSplats;
            job.m_Output = data;
            job.Schedule(inputSplats.Length, 8192).Complete();
        ...
        [BurstCompile]
        struct CreateMaterialDataJob : IJobParallelFor
        {
            [ReadOnly] public NativeArray<InputSplatData> m_Input;
            [NativeDisableParallelForRestriction] public NativeArray<float2> m_Output;

            public void Execute(int index)
            {
                var splat = m_Input[index];
                int i = SplatIndexToTextureIndex((uint)index);
                m_Output[i] = new float2(splat.roughness, splat.metallic);
            }
        }

        void CreateMaterialData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            var (width, height) = GaussianSplatAsset.CalcTextureSize(inputSplats.Length);
            NativeArray<float2> data = new(width * height, Allocator.TempJob);

            CreateMaterialDataJob job = new CreateMaterialDataJob();
            job.m_Input = inputSplats;
            job.m_Output = data;
            job.Schedule(inputSplats.Length, 8192).Complete();

            dataHash.Append(data);
            dataHash.Append((int)m_FormatMat);

            GraphicsFormat gfxFormat = GaussianSplatAsset.MaterialFormatToGraphics(m_FormatMat);
            int dstSize = (int)GraphicsFormatUtility.ComputeMipmapSize(width, height, gfxFormat);

            ConvertMaterialJob jobConvert = new ConvertMaterialJob
            {
                width = width,
                height = height,
                inputData = data,
