...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                mpb.SetInteger(GaussianSplatRenderer.Props.ShaderModeID, (int)gs.m_ShaderMode);
                ...
                {
                    displayMat = gs.m_MatDepth;
                    GaussianSplatRenderer.ShaderMode preMode = gs.m_ShaderMode;
                    gs.m_ShaderMode = GaussianSplatRenderer.ShaderMode.Depth;
                    gs.CalcViewData(cmb, cam);
                    gs.m_ShaderMode = preMode;
                }
        ...
        void OnPreCullCamera(Camera cam)
        {
            ...
            if (cam.CompareTag("Light"))
            {
                m_CommandBuffer.GetTemporaryRT(GaussianSplatRenderer.Props.GaussianSplatRT, 512, 512, 24, FilterMode.Point, GraphicsFormat.R16G16B16A16_SFloat);
            ...
            if (cam.CompareTag("Light"))
            {
                PointLight dst = cam.gameObject.GetComponent<PointLight>();
                m_CommandBuffer.BeginSample(s_ProfCompose);
                m_CommandBuffer.SetRenderTarget(dst.shadowCubemap, 0, dst.currentFace);
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        public ShaderMode m_ShaderMode = ShaderMode.Original;
        ...
        public Cubemap m_EnvPrefilteredMap;
        public Texture m_BRDFLUT;

        public List<PointLight> m_PointLights;
        int m_PrePointCounts;
        GraphicsBuffer m_PointLightData;
        RenderTexture dummyShadowCubeArray;
        public GSEffects m_GSEffects;
        ...
        internal static class Props
        {
            ...
            public static readonly int SplatAlbedo = Shader.PropertyToID("_SplatAlbedo");
            public static readonly int SplatMaterial = Shader.PropertyToID("_SplatMaterial");
            public static readonly int SplatNormal = Shader.PropertyToID("_SplatNormal");
            ...
            public static readonly int ShaderModeID = Shader.PropertyToID("_ShaderMode");
            ...
            public static readonly int IrradianceMap = Shader.PropertyToID("_IrradianceMap");
            public static readonly int PrefilteredMap = Shader.PropertyToID("_PrefilteredMap");
            public static readonly int BRDFLUT = Shader.PropertyToID("_BRDFLUT");

            public static readonly int CurveTexColor = Shader.PropertyToID("_ColorCurveTex");
            public static readonly int CurveTexMetal = Shader.PropertyToID("_MetallicCurveTex");
            public static readonly int CurveTexRough = Shader.PropertyToID("_RoughnessCurveTex");
            ...
            public static readonly int PointLightMap0 = Shader.PropertyToID("_PointLightMap0");
        }
        ...
        void CreateResourcesForAsset()
        {
            ...
            var colorFormat = GaussianSplatAsset.ColorFormatToGraphics(asset.colorFormat);
            var texColor = new Texture2D(texWidth, texHeight, colorFormat, TextureCreationFlags.DontInitializePixels | TextureCreationFlags.IgnoreMipmapLimit | TextureCreationFlags.DontUploadUponCreate) { name = "GaussianColorData" };
            texColor.SetPixelData(asset.colorData.GetData<byte>(), 0);
            texColor.Apply(false, true);
            m_GpuColorData = texColor;

            var texAlbedo = new Texture2D(texWidth, texHeight, colorFormat, TextureCreationFlags.DontInitializePixels | TextureCreationFlags.IgnoreMipmapLimit | TextureCreationFlags.DontUploadUponCreate) { name = "GaussianAlbedoData" };
            texAlbedo.SetPixelData(asset.albedoData.GetData<byte>(), 0);
            texAlbedo.Apply(false, true);
            m_GpuAlbedoData = texAlbedo;

            var matFormat = GaussianSplatAsset.MaterialFormatToGraphics(asset.materialFormat);
            var texMaterial = new Texture2D(texWidth, texHeight, matFormat, TextureCreationFlags.DontInitializePixels | TextureCreationFlags.IgnoreMipmapLimit | TextureCreationFlags.DontUploadUponCreate) { name = "GaussianMaterialData" };
            texMaterial.SetPixelData(asset.materialData.GetData<byte>(), 0);
        ...
        bool resourcesAreSetUp => m_ShaderSplats != null && m_ShaderDepth != null && m_ShaderComposite != null && m_ShaderDebugPoints != null &&
        ...
        public void EnsureMaterials()
        {
            ...
            if(dummyShadowCubeArray == null)
            {
                dummyShadowCubeArray = new RenderTexture(4, 4, 0);
                dummyShadowCubeArray.dimension = UnityEngine.Rendering.TextureDimension.Cube;
                dummyShadowCubeArray.volumeDepth = 6;
                dummyShadowCubeArray.format = RenderTextureFormat.ARGBHalf;
                dummyShadowCubeArray.useMipMap = false;
                dummyShadowCubeArray.autoGenerateMips = false;
                dummyShadowCubeArray.Create();
            }
        }
        ...
        public void UpdateLightDatas()
        {
            if (m_PointLights.Count != m_PrePointCounts)
            {
                m_PointLightData?.Release();
                if (m_PointLights.Count > 0)
                    m_PointLightData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, m_PointLights.Count, 32) { name = "PointLightsData" };
                else
                    m_PointLightData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 1, 32) { name = "PointLightsData" };
                m_PrePointCounts = m_PointLights.Count;
            }
            if(m_PointLightData == null || !m_PointLightData.IsValid())
                m_PointLightData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, Math.Max(1, m_PointLights.Count), 32) { name = "PointLightsData" };

            if (m_PrePointCounts == 0)
            {
                var fakeLight = new PointLight.PointLightData[1];
                m_PointLightData.SetData(fakeLight);

                return;
            }

            var lightDataArray = new PointLight.PointLightData[m_PrePointCounts];
            for (int i = 0; i < m_PrePointCounts; i++)
            {
                PointLight pl = m_PointLights[i];
                lightDataArray[i] = pl.GetData();
            }

            m_PointLightData.SetData(lightDataArray);
        }
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            if (m_GSEffects)
            {
                cmb.SetComputeTextureParam(cs, kernelIndex, Props.CurveTexColor, m_GSEffects.colorCurveTex);
                cmb.SetComputeTextureParam(cs, kernelIndex, Props.CurveTexMetal, m_GSEffects.metallicCurveTex);
                cmb.SetComputeTextureParam(cs, kernelIndex, Props.CurveTexRough, m_GSEffects.roughnessCurveTex);
            }

            cmb.SetComputeTextureParam(cs, kernelIndex, Props.IrradianceMap, m_EnvIrradianceMap);
            cmb.SetComputeTextureParam(cs, kernelIndex, Props.PrefilteredMap, m_EnvPrefilteredMap);
            cmb.SetComputeTextureParam(cs, kernelIndex, Props.BRDFLUT, m_BRDFLUT);

            cmb.SetComputeIntParam(cs, Props.PointLightCount, m_PrePointCounts);
            cmb.SetComputeBufferParam(cs, kernelIndex, Props.PointLights, m_PointLightData);
            if(m_PrePointCounts > 0)
            {
                cmb.SetComputeTextureParam(cs, kernelIndex, Props.PointLightMap0, m_PointLights[0].shadowCubemap);
            }
            else
            {
                cmb.SetComputeTextureParam(cs, kernelIndex, Props.PointLightMap0, dummyShadowCubeArray);
            }
        ...
        internal void SetAssetDataOnMaterial(MaterialPropertyBlock mat)
        {
            ...
            if (m_GSEffects)
            {
                mat.SetTexture(Props.CurveTexColor, m_GSEffects.colorCurveTex);
                mat.SetTexture(Props.CurveTexMetal, m_GSEffects.metallicCurveTex);
                mat.SetTexture(Props.CurveTexRough, m_GSEffects.roughnessCurveTex);
            }


            mat.SetTexture(Props.IrradianceMap, m_EnvIrradianceMap);
            mat.SetTexture(Props.PrefilteredMap, m_EnvPrefilteredMap);
            mat.SetTexture(Props.BRDFLUT, m_BRDFLUT);

            mat.SetInt(Props.PointLightCount, m_PrePointCounts);
            mat.SetBuffer(Props.PointLights, m_PointLightData);
            if (m_PrePointCounts > 0)
            {
                mat.SetTexture(Props.PointLightMap0, m_PointLights[0].shadowCubemap);
            }
            else
            {
                mat.SetTexture(Props.PointLightMap0, dummyShadowCubeArray);
            }
        ...
        internal void CalcViewData(CommandBuffer cmb, Camera cam)
        {
            ...
            cmb.SetComputeIntParam(m_CSSplatUtilities, Props.ShaderModeID, (int)m_ShaderMode);
        ...
        {
            Debug.LogError("EditSetSplatCount() Function has not been modified for GSIR!");
            ...
            int copySrcStartIndex, int copyDstStartIndex, int copyCount)
        {
            ...
            Debug.LogError("EditCopySplats() Function has not been modified for GSIR!");
