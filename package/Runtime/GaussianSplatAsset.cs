...
namespace GaussianSplatting.Runtime
{
    public class GaussianSplatAsset : ScriptableObject
    {
        ...
        public static int GetMaterialSize(MaterialFormat fmt)
        {
            return fmt switch
            {
                MaterialFormat.Float32x2 => 8,
                MaterialFormat.Float16x2 => 4,
                MaterialFormat.Norm8x2 => 2,
                _ => throw new ArgumentOutOfRangeException(nameof(fmt), fmt, null)
            };
        }
        ...
        public void Initialize(int splats, VectorFormat formatPos, VectorFormat formatScale, ColorFormat formatColor, MaterialFormat formatMat, SHFormat formatSh, Vector3 bMin, Vector3 bMax, CameraInfo[] cameraInfos)
        {
            m_SplatCount = splats;
        ...
        public void SetAssetFiles(TextAsset dataChunk, TextAsset dataPos, TextAsset dataNor, TextAsset dataOther, TextAsset dataColor, TextAsset dataAlbedo, TextAsset dataMaterial, TextAsset dataSh)
        {
            m_ChunkData = dataChunk;
            m_PosData = dataPos;
            m_NormalData = dataNor;
            m_OtherData = dataOther;
            m_ColorData = dataColor;
            m_AlbedoData = dataAlbedo;
            m_MaterialData = dataMaterial;
            m_SHData = dataSh;
        }
        ...
        public static GraphicsFormat MaterialFormatToGraphics(MaterialFormat format)
        {
            return format switch
            {
                MaterialFormat.Float32x2 => GraphicsFormat.R32G32_SFloat,
                MaterialFormat.Float16x2 => GraphicsFormat.R16G16_SFloat,
                MaterialFormat.Norm8x2 => GraphicsFormat.R8G8_UNorm,
                _ => throw new ArgumentOutOfRangeException(nameof(format), format, null)
            };
        }
        ...
        public static long CalcMaterialDataSize(int splatCount, MaterialFormat formatMat)
        {
            var (width, height) = CalcTextureSize(splatCount);
            return width * height * GetMaterialSize(formatMat);
        }
        ...
        [SerializeField] MaterialFormat m_MaterialFormat;

        [SerializeField] TextAsset m_PosData;
        [SerializeField] TextAsset m_NormalData;
        [SerializeField] TextAsset m_ColorData;
        [SerializeField] TextAsset m_AlbedoData;
        [SerializeField] TextAsset m_MaterialData;
        ...
        public MaterialFormat materialFormat => m_MaterialFormat;

        public TextAsset posData => m_PosData;
        public TextAsset normalData => m_NormalData;
        public TextAsset colorData => m_ColorData;
        public TextAsset albedoData => m_AlbedoData;
        public TextAsset materialData => m_MaterialData;
